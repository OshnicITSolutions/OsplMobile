...
(function () {
  ...
  angular.module('DJMobileApp.common', []).run(function ($rootScope, $filter, ls,
    pageService, $timeout, $location, $window) {
    //Validating user
    ...
    $rootScope.validateUser = function () {
      ...
      if (token != null) {
        var tokenStr = LZString.decompressFromEncodedURIComponent(token);
